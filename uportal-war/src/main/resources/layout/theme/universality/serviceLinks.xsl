  ...
  <xsl:template name="quicklinks.static.page.bar.links">
    ...
    <xsl:if test="upGroup:isUserDeepMemberOf($USER_ID, 'pags.applicant-undergraduate') or upGroup:isUserDeepMemberOf($USER_ID, 'pags.applicant-graduate') or upGroup:isUserDeepMemberOf($USER_ID, 'pags.student') or upGroup:isUserDeepMemberOfGroupName($USER_ID, 'AT Support') or upGroup:isUserDeepMemberOfGroupName($USER_ID, 'Administrators - ISIS')">
      <span id="portalPageBar_studentcenter">
        <a href="javascript: window.open('{$BASE_ACTION_URL}?uP_fname=student-center&amp;pltc_type=ACTION&amp;pltp_action=loginAction','_new_studentcenter','toolbar=no,directories=no,menubar=yes,resizable=yes,dependent=no,width=800,height=800'); void('');" title="Student&#160;Center" role="menuitem">            <!-- Navigation item link. -->
    ...
    <span id="portalPageBar_mybookmarks">
      <a href="javascript: window.open('{$BASE_ACTION_URL}?uP_fname=my-bookmarks&amp;pltc_type=ACTION&amp;pltc_state=detached','_new_mybookmarks','status=yes,scrollbars=yes,resizable=yes,width=400,height=500'); void('');" title="My&#160;Bookmarks" role="menuitem">  <!-- Navigation item link. -->
          <span>Bookmarks</span>
        </a>
        <xsl:call-template name="portal.pipe" />
    </span>
